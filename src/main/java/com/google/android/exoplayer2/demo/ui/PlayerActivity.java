...
import com.google.android.exoplayer2.demo.utils.DownloadTracker;
...
public class PlayerActivity extends Activity
        implements OnClickListener, PlayerControlView.VisibilityListener, AdEvent.AdEventListener {
    ...
    private boolean isContentPlayCompleted = false;
    ...
    public void onStop() {
        super.onStop();
        if (isAdpause == false && mAdsManager != null) {
            mAdsManager.pause();
        }
    ...
    private void initializePlayer() {
        if (player == null) {
            ...
            RenderersFactory renderersFactory =
                    DemoApplication.buildRenderersFactory(preferExtensionDecoders, getApplicationContext());
            player =
                    new SimpleExoPlayer.Builder(this, renderersFactory).setTrackSelector(trackSelector).build();

            //Conviva-Integration
            String assetName = getIntent().getStringExtra(PlayerActivity.ASSET_NAME);
            Boolean isLive = getIntent().getBooleanExtra(PlayerActivity.IS_LIVE, false);
            ConvivaHelper.onContentPlaybackStart(mConvivaVideoAnalytics, player, getIntent(), assetName, isLive);

            String ad_url = getIntent().getStringExtra(PlayerActivity.AD_TAG_URI);
            if (ad_url != null) {
                ConvivaHelper.initAdSession(mAdsLoader, mConvivaVideoAnalytics, ad_url);
            }
    ...
    private MediaSource buildMediaSource(Uri uri, @Nullable String overrideExtension) {
        DownloadRequest downloadRequest = DemoApplication.getDownloadTracker(getApplicationContext()).getDownloadRequest(uri);
    ...
    private void releasePlayer() {
        if (player != null) {
            ConvivaHelper.release(mConvivaVideoAnalytics);
            releaseAdsLoader();
            updateTrackSelectorParameters();
            updateStartPosition();
    ...
    private DataSource.Factory buildDataSourceFactory() {
        return DemoApplication.buildDataSourceFactory(getApplicationContext());
    }
    ...
    private void releaseAdsLoader() {
        if (mAdsLoader != null) {
            ...
            ConvivaHelper.releaseAdSession();
            mAdsLoader = null;
            if(playerView != null)
    ...
    public void onAdEvent(AdEvent adEvent) {
        ...
        switch (adEvent.getType()) {
            ...
            case ALL_ADS_COMPLETED:
                ...
                if(isContentPlayCompleted){
                    isContentPlayCompleted = false;
                    ConvivaHelper.onContentPlaybackEnded(mConvivaVideoAnalytics);
                }
                break;
            case COMPLETED:
